...
from . import async_websocket_client, game_utils
...
class _GameClient:
  ...
  async def main_loop(self):
    self.websocket = async_websocket_client.AsyncWebsocketClient()
    await self.websocket.handshake(f'wss://{self.server}.adventure.land:2053/socket.io/?EIO=4&transport=websocket')
    while self._running:
      asyncio.create_task(self.update_player_data())
      id, data = game_utils.parse_socketio(await self.websocket.recv())
      if id == 2: await self.websocket.send('3')
      if id == 0: await self.websocket.send('40')
      if id == 42:
        ...
        if data[0] == 'welcome':
          ...
          if not self.sent_auth:
            await self._send_auth()
        if data[0] == "entities" and self.sent_auth:
          self._update_entities(data[1])
        if data[0] == "player" and self.sent_auth:
          self._last_player_update = time.time()
        ...
        if data[0] == "death" and self.sent_auth:
